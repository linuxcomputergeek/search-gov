...
describe VideoNewsSearch do
  ...
  describe '#initialize(options)' do
    it 'should initialize per_page' do
      expect(described_class.new(query: 'gov', tbs: 'w', affiliate: ).per_page).to eq(20)
    end

    it 'should not overwrite per_page option' do
      expect(described_class.new(query: 'gov', tbs: 'w', affiliate: , per_page: '15').per_page).to eq(15)
    end
  end
  ...
  describe '#run' do
    context 'when a valid active RSS feed is specified' do
      it 'should only search for news items from that feed' do
        ...
        search = described_class.new(query: 'element', channel: '100', affiliate: )
