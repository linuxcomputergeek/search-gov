...
describe OdieSearch do
  ...
  describe '#initialize(options)' do
    context "when the query param isn't set" do
      it "should set 'query' to a blank string" do
        expect(described_class.new(affiliate: ).query).to be_blank
      end
    end
  end
  ...
  describe '#cache_key' do
    ...
    it 'should output a key based on the query, affiliate id, doc collection, and page parameters' do
      expect(described_class.new(query: 'element', affiliate: , page: 4, document_collection: @dc).cache_key).to eq("element:#{affiliate.id}:4:#{@dc.id}")
      expect(described_class.new(query: 'element', affiliate: , page: 4).cache_key).to eq("element:#{affiliate.id}:4:")
    end
  end

  describe '#as_json' do
    before do
      @search = described_class.new(query: 'element', affiliate: )
      @search.run
    end
